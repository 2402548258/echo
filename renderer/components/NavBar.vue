...
<template>
    <drag-region>
        <!-- flex排版 如果想h-full并且不超出范围需要在子元素设置margin 不能在父元素mt-[.7rem] mb-[.7rem]-->
        <nav class="h-full flex flex-col justify-between  px-4  border-r border-r-input text-tx-secondary">
            <ul class="mt-[.7rem]">
                <li class="sidebar-item no-drag cursor-pointer hover:text-primary-hover text-tx-primary" :class="{'active':route.name === 'conversation'}">
