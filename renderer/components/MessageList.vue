...
async function handleContextMenu(msgId: number) {
    const clickItem = await createMenu(MENU_IDS.MESSAGE_ITEM);
...
async function handleBatchDelete(){
    ...
    if(res === 'confirm'){
        checkedIds.value.forEach(item => deleteMessage(item))
        quitBatchMode()
    }
}
                ...
                <div class="flex flex-auto"
                    ...
                    <span>
                        ...
                        <div class="msg-shadow p-2 px-6 rounded-md bg-bubble-self text-white"
                            v-if="message.type === 'question'" @contextmenu.stop="handleContextMenu(message.id)">
                        ...
                        }" @contextmenu.stop="handleContextMenu(message.id)">
